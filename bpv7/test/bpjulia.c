...
#if defined (ION_LWT)
int	bpjulia(saddr a1, saddr a2, saddr a3, saddr a4, saddr a5,
...
int	main(int argc, char **argv)
{
	...
	if (ownEid == NULL)
	{
		PUTS("Usage: bpjulia <own endpoint ID>");
		fflush(NULL);
		return 0;
	}
	...
	while (state.running)
	{
		...
		{
			putErrmsg("bpjulia bundle reception failed.", NULL);
			state.running = 0;
			continue;
		}

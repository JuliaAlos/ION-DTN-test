...
bpjulia - Bundle Protocol reception test program

=head1 SYNOPSIS

B<bpjulia> I<ownEndpointId>

=head1 DESCRIPTION

B<bpjulia> uses Bundle Protocol to receive application data units from a
...
B<bpjulia> terminates upon receiving the SIGQUIT signal, i.e., ^C from the
keyboard.
...
B<bpjulia> has terminated.  Any problems encountered during operation
...
Diagnostic messages produced by B<bpjulia> are written to the ION log
file I<ion.log>.
...
=item bpjulia bundle reception failed.
